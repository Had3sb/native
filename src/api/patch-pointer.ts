/**
 * Naming one member of an object in a JMAP PatchObject. Port of the webmail's
 * lib/jmap/patch-pointer.ts.
 *
...
/** The PatchObject key that adds or removes membership of `mailboxId`. */
export function mailboxPointer(mailboxId: string): string {
  return `mailboxIds/${pointerToken(mailboxId)}`;
}
