...
// Decode a UTF-8 byte array to a string. Uses the platform TextDecoder when
// present (faster, charset-aware) and otherwise falls back to a manual UTF-8
// decode so this works under Hermes without a TextDecoder polyfill.
function decodeBytes(bytes: number[], charset?: string): string {
  ...
  if (Decoder) {
    try {
      return new Decoder(charset || 'utf-8').decode(new Uint8Array(bytes));
    } catch {
      try {
        return new Decoder('utf-8').decode(new Uint8Array(bytes));
      } catch {
...
export function parseVcardDate(value: string, params: Record<string, string>): PartialDate | undefined {
    ...
    for (const rawValue of values) {
      ...
      switch (propName) {
        ...
        case 'CATEGORIES': {
          ...
          const cats = val.split(',').map((cat) => cat.trim()).filter(Boolean);
...
function generateSingleVCard(contact: ContactCard): string {
  ...
  if (contact.organizations) {
    for (const org of Object.values(contact.organizations)) {
      if (!org.name && !(org.units && org.units.length)) continue;
  ...
  if (contact.addresses) {
    for (const addr of Object.values(contact.addresses)) {
      ...
      const parts = ['', '', street, locality, region, postcode, country];
  ...
  for (const c of existing) {
    if (c.emails) {
      for (const e of Object.values(c.emails)) {
        if (e.address) existingEmails.set(e.address.toLowerCase(), c.id);
      }
    }
  }
...
// ─── Backward-compatible single/multi export helpers ─────────────
// Older call sites (ContactDetailScreen, ContactsSettings) import these names.
