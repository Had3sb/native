 ...
 * Port of the webmail's lib/sub-addressing.ts.
 *
...
/**
 * Parse an email address to extract the sub-address tag. The first occurrence
 * of the delimiter in the local part separates the base user from the tag,
 * matching the behavior of Postfix/qmail/Sieve.
 */
export function parseSubAddress(
  ...
  if (delimiterIndex === -1) {
    return { localPart, baseUser: localPart, tag: null, domain, fullAddress: email };
  }

  const baseUser = localPart.substring(0, delimiterIndex);
  const tag = localPart.substring(delimiterIndex + delimiter.length);

  return { localPart, baseUser, tag: tag || null, domain, fullAddress: email };
}
...
/** Suggest tags based on the recipient domain. */
...
/**
 * Get validation error code for an invalid tag. Returns an error code that
 * should be translated by the calling component.
 */
export function getTagValidationError(tag: string): TagValidationErrorCode {
  if (!tag) return 'EMPTY';
  if (tag.length > MAX_TAG_LENGTH) return 'TOO_LONG';
  if (!/^[a-zA-Z0-9-]+$/.test(tag)) return 'INVALID_CHARS';
  return null;
}
