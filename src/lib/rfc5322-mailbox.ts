 ...
 * Port of the webmail's lib/rfc5322-mailbox.ts.
 *
 ...
 * (`Jane <j@x.com>`, `Jane<j@x.com` → `Jane`). Line breaks are collapsed so a
 * pasted name can never inject a header.
 */
export function sanitizeDisplayName(name: string | undefined | null): string {
  if (!name) return '';
  const flat = name.replace(/[\r\n\t]+/g, ' ');
  return unquoteDisplayName(unquoteDisplayName(flat).replace(/\s*<[^<>]*>?\s*$/, ''));
}
