 ...
 * should be hidden from the *event* calendar UI (#761 / #28).
 *
 ...
 * Port of the webmail's lib/calendar-component-detection.ts.
 */
...
/** The properties a tasks-only scan needs to request. */
export const SCAN_PROPERTIES = ['id', '@type', 'due', 'progress', 'percentComplete', 'calendarIds'];
 ...
 * `percentComplete`) - a VEVENT never carries those. Thunderbird/Todoist
 * VTODOs that Stalwart returns without `@type` are otherwise neither shown
 * as tasks nor counted for tasks-only calendars.
 */
