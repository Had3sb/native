...
- [ ] **No request deadline: a stalled socket hangs send/save forever (#702)** — `P1` — `bugfix-parity`
...
- [ ] **`maxConcurrentRequests` refusal (400 `jmap:error:limit`) is a hard failure (#780)** — `P2` — `bugfix-parity`
...
- [ ] **No transient-network retry** — `P2` — `missing`
...
- [ ] **429 handling: no rate-limit state, no pause of live updates, HTTP-date `Retry-After` becomes NaN** — `P2` — `partial`
...
- [ ] **HTTP error body and cause not surfaced; JSON parse unguarded** — `P3` — `partial`
...
- [ ] **Method-level `error` / `notUpdated` / `notDestroyed` unchecked in most RN API functions** — `P1` — `rn-only-bug`
...
- [ ] **`Email/get` and `Email/set` not split to `maxObjectsInGet`/`maxObjectsInSet`; no `getMaxObjectsInSet`** — `P2` — `bugfix-parity`
...
- [ ] **`maxSizeUpload` / `maxSizeRequest` ignored** — `P3` — `missing`
...
- [ ] **Relative session URLs are not resolved (37f830da)** — `P2` — `bugfix-parity`
...
- [ ] **PatchObject pointer keys not JSON-Pointer escaped (5c484af1)** — `P3` — `bugfix-parity`
...
- [ ] **Send: message created directly in Sent instead of Drafts + `onSuccessUpdateEmail` (#188)** — `P2` — `bugfix-parity`
...
- [ ] **Message-ID / In-Reply-To handling diverges from WEB** — `P3` — `bugfix-parity`
...
- [ ] **No keep-alive ping / connection-lost signalling / session refresh** — `P2` — `missing`
...
- [ ] **Password change leaves the stored credential stale, then evicts the account** — `P1` — `rn-only-bug`
...
- [ ] **First-touch serialisation for Calendar/Contacts (#907)** — `P3` — `bugfix-parity`
...
- [ ] **Session redirect with dropped Authorization not handled** — `P3` — `partial`
...
- [ ] **No `AppState` handling: SSE never paused in background nor recovered on foreground; no refresh on resume** — `P1` — `missing`
...
- [ ] **SSE Authorization header captured once; stale after OAuth token refresh** — `P1` — `rn-only-bug`
...
- [ ] **Push effect keyed on the singleton `client`: SSE stays bound to the previous account after `switchAccount`** — `P1` — `rn-only-bug`
...
- [ ] **No SSE failure/fallback path (server 4xx/5xx, proxy without SSE, client cert)** — `P2` — `partial`
...
- [ ] **StateChange coverage: EmailSubmission, SieveScript, Identity, VacationResponse, FileNode ignored** — `P2` — `partial`
...
- [ ] **Polling fallback is coarse: primary account only, fixed 5 s, no in-flight coalescing, no pause in background** — `P3` — `partial`
...
- [ ] **SSE stream keeps the whole response in memory; no periodic recycle** — `P3` — `rn-only-bug`
...
- [ ] **Password handoff over a custom-scheme redirect (`bulwarkmobile://…#password=`) is interceptable; OAuth `state` uses `Math.random`** — `P1` — `rn-only-bug`
...
- [ ] **`http://` server URLs accepted without a warning; cleartext then fails opaquely** — `P2` — `missing`
...
- [ ] **Weak randomness for TOTP secret / UUIDs (falls back to `Math.random`)** — `P2` — `rn-only-bug`
...
- [ ] **Client-cert native fetch follows redirects with Authorization and turns POST into GET (#627 class)** — `P3` — `bugfix-parity`
...
- [ ] **Client-cert path buffers whole bodies through base64 over the bridge with a 30 s cap** — `P3` — `rn-only-bug`
...
- [ ] **TOTP helper: RFC 6238 compatible; issuer hard-coded** — `P3` — `partial`
...
- [ ] **Encryption at rest is read-only; no public-key management** — `P2` — `partial`
...
- [ ] **Display name not refreshed from the Stalwart principal on login/restore/switch (#900)** — `P3` — `bugfix-parity`
...
- [ ] **Native issue #47 "Account security not accessible" - likely gating** — `P3` — `rn-only-bug` (unverified)
