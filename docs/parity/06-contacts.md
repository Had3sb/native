...
- [ ] **Newly created contact appears as a blank row until refresh** — `P2` — `bug`
...
- [ ] **`ContactCard/set` errors are swallowed or crash with a TypeError** — `P2` — `bug`
...
- [ ] **No UID assigned on contact creation (#644)** — `P2` — `bugfix-parity`
...
- [ ] **Contact list silently truncated at the query limit (no pagination)** — `P2` — `bug`
...
- [ ] **No first-touch gate: concurrent AddressBook/get + ContactCard/query on cold start (#907)** — `P2` — `bugfix-parity`
...
- [ ] **Contacts account id: RN uses the mail primary account, not `primaryAccounts[contacts]`** — `P3` — `partial`
...
- [ ] **Shared / group-account address books not listed or writable** — `P2` — `missing`
...
- [ ] **Deleting a contact does not clean group member references locally** — `P3` — `partial`
...
- [ ] **Deleting an address book leaves its contacts in the cache** — `P3` — `partial`
...
- [ ] **Persisting every contact card (incl. base64 photos) to AsyncStorage** — `P3` — `rn-only-bug`
...
- [ ] **Set default address book missing** — `P3` — `missing`
...
- [ ] **Default address book can be offered for deletion** — `P3` — `partial`
...
- [ ] **Clearing a whole field group on edit never reaches the server** — `P2` — `rn-only-bug`
...
- [ ] **Saving the form drops non-photo media (logo, sound)** — `P3` — `rn-only-bug`
...
- [ ] **Anniversary input accepts free text that the server rejects; year-only dates get a fake Jan 1** — `P2` — `rn-only-bug`
...
- [ ] **Organization-kind cards (#701)** — `P3` — `partial`
...
- [ ] **Photo data URIs from Stalwart not normalised (#307)** — `P2` — `bugfix-parity`
...
- [ ] **Picked photos are stored full-size** — `P3` — `partial`
...
- [ ] **Duplicate contact keeps the source UID** — `P2` — `rn-only-bug`
...
- [ ] **Gender vocabulary differs from WEB and from the vCard exporter** — `P3` — `partial`
...
- [ ] **Online service `label` and multiple nicknames not editable** — `P3` — `partial`
...
- [ ] **Display name includes the middle name (differs from WEB sorting/grouping)** — `P3` — `partial`
...
- [ ] **Alphabetical index buckets every non-ASCII initial under "#"** — `P3` — `bug`
...
- [ ] **"Uncategorized" means "no address book" in RN but "no category" in WEB, and its count is hard-coded 0** — `P3` — `bug`
...
- [ ] **No select-all / bulk export in multi-select** — `P3` — `partial`
...
- [ ] **Rename category (keyword) missing** — `P3` — `missing`
...
- [ ] **Add-to-group from the contact itself missing** — `P3` — `partial`
...
- [ ] **No way to create a contact group** — `P2` — `missing`
...
- [ ] **Group edit reuses the full person form (name split into first/last, photo, emails…)** — `P3` — `partial`
...
- [ ] **Group compose: RN expands to individual recipients; no To/Cc/Bcc choice** — `P3` — `partial`
...
- [ ] **Imported dates are raw strings, not RFC 9553 PartialDate (#224) — birthdays vanish on Stalwart** — `P1` — `bugfix-parity`
...
- [ ] **Vendor X- extensions not imported (X-ABDATE, X-ABRELATEDNAMES, X-SPOUSE/MANAGER/ASSISTANT, X-MAIDENNAME, X-GENDER, X-ANDROID-CUSTOM, X-AIM/X-SKYPE/… IM handles)** — `P2` — `bugfix-parity`
...
- [ ] **Apple grouped labels (`item1.TEL` + `item1.X-ABLABEL`) are lost** — `P2` — `bugfix-parity`
...
- [ ] **Org-only vCards are dropped on import and exported without FN (#701)** — `P2` — `bugfix-parity`
...
- [ ] **Only one NICKNAME survives import** — `P3` — `bugfix-parity`
...
- [ ] **Import target defaults to the first book and cannot be changed from Settings** — `P3` — `partial`
...
- [ ] **Import keeps the source UID (fine) but nothing mints one when absent** — covered by the #644 finding above; also `importContacts` swallows per-card failures with only a `console.warn` (`src/stores/contacts-store.ts:206-208`) so "Imported 0 contacts" is the only feedback. `P3` — add a failure count to the result sheet.

### Trusted senders

- [ ] **Trusted-senders settings show only the local list; synced book entries are invisible and cannot be removed** — `P2` — `partial`
...
- [ ] **`trustedSendersAddressBook` setting is dead (`false`, no setter, no toggle) — book is consulted unconditionally** — `P3` — `partial`
...
- [ ] **Race: trusting a sender while the passive book load is in flight silently fails** — `P2` — `rn-only-bug`
...
- [ ] **Duplicate "Trusted Senders" books are not consolidated and the pick is non-deterministic (#730)** — `P3` — `bugfix-parity`
...
- [ ] **Trusted-sender card stores no display name; "Name <email>" input not parsed** — `P3` — `partial`
...
- [ ] **Contact activity: upcoming events come only from the already-loaded calendar cache** — `P3` — `partial`
