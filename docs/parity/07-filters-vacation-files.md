...
- [ ] **RN generator corrupts WEB-authored attachment rules on any save** — `P1` — `bug` (missing feature with data-loss side effect)
...
- [ ] **Multi-value conditions (string[] values) unsupported → WEB scripts become opaque on RN** — `P1` — `missing`
...
- [ ] **FilterRuleModal never re-initialises: editing rule B shows/saves rule A's fields** — `P1` — `rn-only-bug`
...
- [ ] **Sieve test files have mojibake in test names** — `P3` — `rn-only-bug`
...
- [ ] **Raw Sieve editor lacks line-number gutter / Tab-inserts-spaces** — `P3` — `partial` (mostly N/A on mobile)
  - What WEB does: gutter with line numbers, Tab inserts two spaces (`components/filters/sieve-editor-modal.tsx:61-72`, `101-128`).
  - What RN does: shows "N lines" below the editor (`src/components/filters/SieveEditorSheet.tsx:107`). Acceptable on mobile; listed so nobody re-audits it.
...
- Drag-to-reorder filter rules (RN uses chevrons), line-number gutter in the Sieve editor.
