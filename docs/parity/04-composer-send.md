...
- [ ] **No draft autosave / save-on-close — closing a compose loses everything** — `P1` — `missing`
...
- [ ] **Cannot open / edit an existing draft** — `P1` — `missing`
...
- [ ] **Draft keeps In-Reply-To/References and reply-mode state** — `P2` — `missing`
...
- [ ] **Undo-send delay has no undo toast / "send now"** — `P2` — `partial`
...
- [ ] **Read-receipt (MDN) request missing** — `P3` — `missing`
...
- [ ] **Sending from shared/group accounts unsupported** — `P3` — `missing` (partly N/A)
...
- [ ] **Dead duplicate send implementation** — `P3` — `rn-only-bug`
...
- [ ] **Cc addresses are not validated; `Name <addr` recovery and quoted names missing** — `P3` — `bugfix-parity` (changelog 1.9.0 "Split recipient lists whose angle brackets never close", 1.7.x #672)
...
- [ ] **Contact groups not offered as recipients; no server (Sent-folder) search** — `P3` — `missing`
...
- [ ] **Recipient chips show no avatar / no "Name <addr>" tooltip; no chip editing** — `P3` — `partial` (N/A for drag-reorder)
...
- [ ] **Sub-addressing (`user+tag@`) missing** — `P3` — `missing`
...
- [ ] **From override / catch-all alias on an owned domain missing** — `P3` — `missing` (changelog 1.5.x #246)
...
- [ ] **Identity preselection does not consider Bcc or `+tag` and never uses the primary-identity setting** — `P3` — `partial`
...
- [ ] **Identity display name not sanitized on create/update** — `P3` — `bugfix-parity` (changelog 1.5.x "Sanitize identity display name to prevent invalid From headers")
...
- [ ] **Identity editor lacks HTML signature, Reply-To, Bcc fields and size caps** — `P3` — `partial`
...
- [ ] **Identity sync (visibility/interval refresh) missing** — `P3` — `partial`
...
- [ ] **Signatures are never inserted or sent** — `P2` — `missing`
...
- [ ] **Signature settings (position, separator) absent** — `P3` — `missing`
...
- [ ] **Reply / forward quotes plain text only — HTML layout and inline images are lost** — `P2` — `missing` (changelog #163, #543, "Editable, layout-preserving quote island")
...
- [ ] **Quote header is English-only and shows the sender name without address** — `P3` — `partial` (changelog #482, i18n quote header)
...
- [ ] **Attachment reminder scans the quoted original** — `P3` — `bugfix-parity` (changelog #570)
...
- [ ] **Empty subject blocks Send instead of confirming** — `P3` — `bugfix-parity` (changelog #684)
...
- [ ] **Toolbar coverage: no text color, tables, font, horizontal rule, text-direction** — `P3` — `partial`
...
- [ ] **Plain-text mode still renders the rich editor and derives text from HTML (links lose their href)** — `P3` — `partial`
...
- [ ] **Inline image insert: no camera, no paste, no multi-select; not offered for file attachments either** — `P3` — `partial`
...
- [ ] **Send-time editor readback (RN #9/#11) — verified robust, one edge** — `P3` — `rn-only-bug`
...
- [ ] **No byte progress, cancel does not abort the transfer, no size-limit check** — `P2` — `partial` (changelog 1.9.0 "Real byte progress…cancel aborts", 1652a0ec "respect server limits")
...
- [ ] **Attachment chip has no preview / open** — `P3` — `missing`
...
- [ ] **Scheduled screen: cancel only — no reschedule, no "send now", no edit** — `P2` — `partial`
...
- [ ] **Scheduled screen strings are hard-coded English** — `P3` — `rn-only-bug`
...
- [ ] **Scheduled preset times ignore the user's 12/24h format** — `P3` — `rn-only-bug`
...
- [ ] **No way to insert a template in the RN composer** — `P2` — `missing`
...
- [ ] **Template model lacks isHTML, category UI, favourites, default recipients, identity; export format differs** — `P3` — `partial`
...
- [ ] **Templates not in cross-device settings sync (#825)** — `P3` — `missing` (blocked)
...
- [ ] **No `mailto:` handling / share-to-app intent** — `P2` — `missing`
...
- [ ] **S/MIME sign/encrypt on send is a stub** — `P3` — `missing`
...
- [ ] **Auto-add reply recipients to trusted senders** — `P3` — `missing` (changelog 1.5.x)
...
- [ ] **Offline send is not queued** — `P3` — `missing` (RN-only opportunity)
...
- [ ] **Permission / picker error strings are not localized** — `P3` — `rn-only-bug`
